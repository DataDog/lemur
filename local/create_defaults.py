...
def create_user(username, password, email, role_names):
    ...
    user = user_service.create(username=username, password=password, email=email, active=True, roles=roles, profile_picture="")
    print(f" # Default user '{username}' created with roles {', '.join(role_names)} (ID: {user.id})")
