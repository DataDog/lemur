from lemur.common.utils import parse_certificate

VALID_USER_HEADER_TOKEN = {
    "Authorization": "Basic "
    + "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE1MjE2NTIwMjIsImV4cCI6MjM4NTY1MjAyMiwic3ViIjoiMSJ9.UQBQzJHyUJNgTVKQRHsBw3tz-plqRWAXiaSIYnOfY9g",
    "Content-Type": "application/json",
}


VALID_ADMIN_HEADER_TOKEN = {
    "Authorization": "Basic "
    + "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE1MjE2NTE2NjMsInN1YiI6IjIiLCJhaWQiOjF9.iwffQd6msR9Qpythn7IHzk1LpZtz7rLWcdP3ECL1R9I",
    "Content-Type": "application/json",
}


VALID_ADMIN_API_TOKEN = {
    "Authorization": "Basic "
    + "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIyIiwiYWlkIjoxLCJpYXQiOjE0MzUyMzMzNjl9.G4LPxutGiMAJBQfVxVqfECplVOguiDHHQSu0z32q3-0",
    "Content-Type": "application/json",
}


#: CN=LemurTrust Unittests Root CA 2018
ROOTCA_CERT_STR = """\
-----BEGIN CERTIFICATE-----

n7K1z+W0Kp5yhnnPAoOioAP4vjASDx3z3RnLaZvMmcO7YdCIwhE5oGV0
-----END CERTIFICATE-----
"""
ROOTCA_CERT = parse_certificate(ROOTCA_CERT_STR)
ROOTCA_KEY = """\
-----BEGIN RSA PRIVATE KEY-----

45ZXINAgovSzBcZfwStWjebUDw4v10Arc6ipkHGBPbK556S6wXGK
-----END RSA PRIVATE KEY-----
"""


#: CN=LemurTrust Unittests Class 1 CA 2018
INTERMEDIATE_CERT_STR = """\
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
INTERMEDIATE_CERT = parse_certificate(INTERMEDIATE_CERT_STR)
INTERMEDIATE_KEY = """\
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""


#: CN=san.example.org, issued by LemurTrust Unittests Class 1 CA 2018
SAN_CERT_CSR = """\
-----BEGIN CERTIFICATE REQUEST-----

ZhDsC0fjqSX0FJKz6gOhP88bkbbapyHuGB71o2dwhCKV
-----END CERTIFICATE REQUEST-----
"""

SAN_CERT_STR = """\
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
SAN_CERT = parse_certificate(SAN_CERT_STR)
SAN_CERT_KEY = """\
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""

WILDCARD_CERT_STR = """\
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
WILDCARD_CERT = parse_certificate(WILDCARD_CERT_STR)
WILDCARD_CERT_KEY = """\
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""


INVALID_CERT_STR = """
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
INVALID_CERT = parse_certificate(INVALID_CERT_STR)


EXTERNAL_VALID_STR = """
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
EXTERNAL_CERT = parse_certificate(EXTERNAL_VALID_STR)


INTERNAL_CERTIFICATE_A_STR = """
-----BEGIN CERTIFICATE-----

L0Ew8hy0GG3nZ6uXLW7q
-----END CERTIFICATE-----
"""


INTERNAL_PRIVATE_KEY_A_STR = """
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""


CSR_STR = """
-----BEGIN CERTIFICATE REQUEST-----

-----END CERTIFICATE REQUEST-----
"""


EC_CERT_STR = """
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
EC_CERT_EXAMPLE = parse_certificate(EC_CERT_STR)


ECDSA_PRIME256V1_CERT_STR = """
-----BEGIN CERTIFICATE-----

Vl89McYhmV4IV7WDgUmUhkUSFXgy
-----END CERTIFICATE-----
"""
ECDSA_PRIME256V1_CERT = parse_certificate(ECDSA_PRIME256V1_CERT_STR)


ECDSA_SECP384r1_CERT_STR = """
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
ECDSA_SECP384r1_CERT = parse_certificate(ECDSA_SECP384r1_CERT_STR)

DSA_CERT_STR = """
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
DSA_CERT = parse_certificate(DSA_CERT_STR)


CERT_CHAIN_PKCS7_STR = """
-----BEGIN PKCS7-----

-----END PKCS7-----
"""

CERT_CHAIN_PKCS7_PEM = CERT_CHAIN_PKCS7_STR.encode("utf-8")

ACME_CHAIN_X1_STR = """-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""


ACME_CHAIN_LONG_STR = (
    SAN_CERT_STR
    + ACME_CHAIN_X1_STR
    + """-----BEGIN CERTIFICATE-----

oZ2lS38fL18Aon458fbc0BPHtenfhKj5
-----END CERTIFICATE-----
"""
)

ACME_CHAIN_SHORT_STR = (
    SAN_CERT_STR
    + """
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
)

IP_SAN_NO_CN_CERT_STR = """
-----BEGIN CERTIFICATE-----

sBWmooFdbP7Hc7KCHOLeH4zJPcEGr1O4UGuwO9JAXqNoKIBEQwix
-----END CERTIFICATE-----
"""


# =============================================================================
# Cross-signed certificate hierarchy for non-linear chain tests (RDNA-926)
#
# Hierarchy:
#   Test Root A (self-signed)
#   Test Root B (self-signed)
#   Test Intermediate (same key, two certs: signed by Root A and by Root B)
#   Test Leaf (signed by Test Intermediate)
#   Unrelated CA (orphan, not connected to the hierarchy)
#   Root A cross-signed by Root B (same key as Root A, signed by Root B)
# =============================================================================

#: CN=Test Root A (self-signed)
CROSS_SIGNED_ROOT_A_CERT_STR = """\
-----BEGIN CERTIFICATE-----

/KOE
-----END CERTIFICATE-----
"""
CROSS_SIGNED_ROOT_A_CERT = parse_certificate(CROSS_SIGNED_ROOT_A_CERT_STR)

CROSS_SIGNED_ROOT_A_KEY = """\
-----BEGIN RSA PRIVATE KEY-----

OB2ShAiVAOYaz0gMyHgk0A/n529kO5LxXKXJHnkwf1sbbSEIQjBw
-----END RSA PRIVATE KEY-----
"""

#: CN=Test Root B (self-signed)
CROSS_SIGNED_ROOT_B_CERT_STR = """\
-----BEGIN CERTIFICATE-----

lCoO
-----END CERTIFICATE-----
"""
CROSS_SIGNED_ROOT_B_CERT = parse_certificate(CROSS_SIGNED_ROOT_B_CERT_STR)

#: CN=Test Intermediate, signed by Test Root A
CROSS_SIGNED_INT_BY_A_CERT_STR = """\
-----BEGIN CERTIFICATE-----

GrBqaW+eblL7Uymb
-----END CERTIFICATE-----
"""
CROSS_SIGNED_INT_BY_A_CERT = parse_certificate(CROSS_SIGNED_INT_BY_A_CERT_STR)

#: CN=Test Intermediate, signed by Test Root B (same key as INT_BY_A)
CROSS_SIGNED_INT_BY_B_CERT_STR = """\
-----BEGIN CERTIFICATE-----

8sSsAQlB8peGNi8v
-----END CERTIFICATE-----
"""
CROSS_SIGNED_INT_BY_B_CERT = parse_certificate(CROSS_SIGNED_INT_BY_B_CERT_STR)

CROSS_SIGNED_INT_KEY = """\
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""

#: CN=test.example.com, signed by Test Intermediate
CROSS_SIGNED_LEAF_CERT_STR = """\
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
CROSS_SIGNED_LEAF_CERT = parse_certificate(CROSS_SIGNED_LEAF_CERT_STR)

CROSS_SIGNED_LEAF_KEY = """\
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
"""

#: CN=Unrelated CA (self-signed, not connected to the cross-signed hierarchy)
ORPHAN_CERT_STR = """\
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"""
ORPHAN_CERT = parse_certificate(ORPHAN_CERT_STR)

#: CN=Test Root A, signed by Test Root B (cross-signed root, same key as ROOT_A)
ROOT_A_CROSS_SIGNED_BY_B_CERT_STR = """\
-----BEGIN CERTIFICATE-----

ZOWq
-----END CERTIFICATE-----
"""
ROOT_A_CROSS_SIGNED_BY_B_CERT = parse_certificate(ROOT_A_CROSS_SIGNED_BY_B_CERT_STR)
